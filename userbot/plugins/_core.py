...
from . import *
DELETE_TIMEOUT = 5
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "『🐼  🎀  𝒟𝒜𝑅𝒦𝑀𝒜𝑀𝐵𝒜  🎀  🐼』"
darkmamba = bot.uid
...
async def send(event):
    ...
    omk = f"**⍟ 𝙿𝚕𝚞𝚐𝚒𝚗 𝚗𝚊𝚖𝚎 ≈** `{input_str}`\n**⍟ 𝚄𝚙𝚕𝚘𝚊𝚍𝚎𝚍 𝙱𝚢 ≈** {darkmamba_mention}\n\n⚡ **[🐼  🎀  𝒟𝒜𝑅𝒦𝑀𝒜𝑀𝐵𝒜  🎀  🐼](https://t.me/DARKMAMBA_SUPPORT)** ⚡"
...
async def install(event):
    ...
    if len(OUTPUT) > 69:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            ...
            await edit_or_reply(PYTHON_file, f"**Output Too Large. This is the file for the list of plugins in 🐼  🎀  𝒟𝒜𝑅𝒦𝑀𝒜𝑀𝐵𝒜  🎀  🐼 .\n\n**BY :- {DEFAULTUSER}**")
            await event.delete()
